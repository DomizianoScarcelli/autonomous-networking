...
class QlearningStepwiseRouting(BASE_routing):
    ...
    def compute_reward(self, old_action):
        ...
        if distance_cur_drone_depot <= config.DEPOT_COMMUNICATION_RANGE:
            ...
            if self.drone.identifier in self.simulator.depot.nodes_table.nodes_list: #In this case the drone is linked with the depot and hop != None
                hop = self.simulator.depot.nodes_table.nodes_list[self.drone.identifier].hop_count
                reward = self.OMEGA*np.exp(1/hop) + (1-self.OMEGA)*self.drone.link_stabilities[self.simulator.cur_step][old_action.identifier]
                if reward > self.RMAX:
                    self.RMAX = reward
                if reward < self.RMIN:
                    self.RMIX = reward
            else:
                reward = self.drone.link_stabilities[self.simulator.cur_step][old_action.identifier]
            """
        #if self.old_state != None and reward != 0:
        #    self.update_qtable(self.old_state.identifier, self.drone.identifier, self.drone.identifier, reward)
    ...
    def relay_selection(self):
        ...
        for neighbor in self.drone.neighbor_table.neighbors_list:
            ...
            link_stability_ij = (1-config.BETA)*np.exp(1/drones_speed[neighbor.identifier])+config.BETA*(link_quality_sum[neighbor.identifier]/self.simulator.n_drones) #Computes the link stability between self and the neighbor
    ...
    def update_link_quality(self):
        #if self.simulator.cur_step in self.link_qualities:
        #    if self.drone.identifier in self.link_qualities[self.simulator.cur_step]: return
        ...
        for step in range(starting_point, self.simulator.cur_step+1): #Finally we assign the computed link qualities to each step from starting point to the current step.
            if not self.simulator.cur_step in self.link_qualities:
    ...
    def sum_n_last_link_qualities(self, drone):
        ...
        """
        return sum([link_quality_by_step[drone.identifier] for link_quality_by_step in self.drone.link_qualities.values()])
